...
@Service
public class FakeMercadoPagoAdapter implements PaymentPort {

    private static final Logger log = LoggerFactory.getLogger(FakeMercadoPagoAdapter.class);
    ...
    public PaymentResult processPayment(BigDecimal amount) {
        ...
        if (failure) {
            log.warn("[MercadoPago] Payment declined. TransactionId: {}, Amount: {}, Reason: Simulated provider rejection",
                    transactionId, amount);
            return new PaymentResult(false, transactionId, "Simulated provider rejection");
        }

        log.info("[MercadoPago] Payment processed. TransactionId: {}, Amount: {}", transactionId, amount);
        return new PaymentResult(true, transactionId, null);
    }

    private PaymentResult paymentFallback(BigDecimal amount, Throwable t) {
        log.error("[MercadoPago] Circuit breaker open. Error: {}", t.getMessage());
        throw new PaymentServiceUnavailableException("mercadopago");
    }
}
